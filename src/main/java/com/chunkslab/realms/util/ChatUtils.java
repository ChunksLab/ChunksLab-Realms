...
public class ChatUtils {
    ...
    private final static MiniMessage MINI_MESSAGE = MiniMessage.builder()
            ...
            .preProcessor(s -> s.replace("<prefix>", RealmsPlugin.getInstance().getPluginMessages().getPrefix()))
    ...
    public static void sendMessage(CommandSender player, Component component) {
        RealmsPlugin.getInstance().getAdventure().sender(player).sendMessage(component);
    }

    public static void sendTitle(CommandSender player, Component title, Component subtitle) {
        RealmsPlugin.getInstance().getAdventure().sender(player).showTitle(Title.title(title, subtitle));
    }
