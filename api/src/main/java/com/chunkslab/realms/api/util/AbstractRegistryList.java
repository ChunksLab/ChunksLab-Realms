...
public abstract class AbstractRegistryList<T extends Keyed> {
    ...
    public boolean contains(@NotNull String element) {
        ...
        final T matched = matchRegistry(this.registry, element);
    ...
    public static class Builder<T extends Keyed, F extends AbstractRegistryList<T>> {
        ...
        public Builder<T, F> add(@NotNull String... names) {
            for (String name : names) {
                final T match = matchRegistry(this.registry, name);
                if (match != null)
                    anyMatchPredicates.add(t -> t.equals(match));
                else {
                    try {
                        System.out.println("Expected element with name '" + name + "' was not found in the " + this.clazz.getSimpleName() + " registry.");
